...
export default async function createSegment(segmentData) {
  const [segment] = await db.insert(segments).values(segmentData).returning();
  return segment;
}

export default async function getSegmentById(id) {
  const [segment] = await db.select().from(segments).where(eq(segments.id, id));
  return segment || null;
}

export default async function getAllSegments(options = {}) {
...
export default async function getSegmentByEvent(event) {
  const [segment] = await db.select().from(segments).where(eq(segments.event, event));
  return segment || null;
}

export default async function updateSegment(id, data) {
...
export default async function deleteSegment(id) {
