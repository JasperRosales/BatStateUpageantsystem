...
export function ScoringList({
  ...
  const roleColors = {
    ...
    MS: "bg-pink-100 text-pink-800",
  };

  return (
    <div className="bg-card text-card-foreground border border-border shadow-2xs rounded-lg overflow-hidden">
      <div className={`${
        role === "MR" ? "bg-blue-700/90" : "bg-pink-700/90"
      } shadow-2xs px-4 py-3 border-b border-border`}>
        ...
        {contestants.length === 0 ? (
          ...
          contestants.map((contestant) => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium text-white text-shadow-2xs shadow-2xs ${
                  role === "MR" ? "bg-blue-700/90" : "bg-pink-700/90"
                }`}>
                  {contestant.number}
                </div>
                <div className="flex-1 font-medium min-w-0">
                  ...
                  <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium mt-1 ${roleColors[role]}`}>
                    {role === "MR" ? "Mr." : "Ms."} BatStateU
                  </span>
                </div>
